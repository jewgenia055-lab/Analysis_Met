...
st.title("Предварительная обработка значений - проверка наличия выбросов")

st.markdown(f"По **ГОСТ 1497-84** площадь поперечного сечения образцов ${r'F_0'}$ до приложения нагрузки допускается определять по номинальным размерам при условии соответствия отклонениий таблице. Ссылка :")
